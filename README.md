...
- **The Tentacles:** Flexible, weighted appendages dangling beneath the dome into the water stream. Each tentacle houses a specific environmental probe (pH, Turbidity, TDS, Temperature). This bio-inspired design prevents debris entanglement and ensures sensors capture readings at various optimal depths.
...
1. **The Jellyfish Node (ESP32 #1):** Collects the physical variables (`pH`, `TDS`, `Turbidity`, `Temp`) and broadcasts them to the remote server.
...
- `models/` - Directory where `.pkl` and `.keras` model artifacts are saved and hot-loaded by the server.
...
- **Constrained Classes:** 
  ...
  - `2`: Antibiotic Contamination (Aggressive pH volatility)
  - `3`: Systemic Anomaly (Extreme randomized metrics simulating probe failure or mass-dumping)
- **Deep Learning Sequence:** The LSTM framework natively consumes a tumbling inference window of `10` sequence-steps across all `4` sensor features before outputting its catastrophic spike probability (`loss='mse'`).
